...
import static com.blueshark.music.theme.Theme.getBaseDialog;
...
public class FolderChooserDialog extends DialogFragment implements MaterialDialog.ListCallback {
  ...
  File[] listFiles() {
    ...
    if (contents != null) {
      for (File fi : contents) {
        if ("/storage/self".equals(fi.getPath())) continue;
        if (fi.isDirectory()) {
          results.add(fi);
        }
      }
  ...
  public void onSelection(MaterialDialog materialDialog, View view, int i, CharSequence s) {
    if (canGoUp && i == 0) {
      ...
      canGoUp = parentFolder.getParent() != null && !parentFolder.getParent().equals("/");
    } else {
