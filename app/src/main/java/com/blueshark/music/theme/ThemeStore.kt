package com.blueshark.music.theme
...
object ThemeStore {
  private const val KEY_NAME = "bsplayer-theme"
  ...
  @JvmField
  var sImmersiveMode: Boolean = true
  ...
  val themeRes: Int
    get() = when (theme) {
      LIGHT -> R.style.Theme_BSPlayer
      BLACK -> R.style.Theme_BSPlayer_Black
      DARK -> R.style.Theme_BSPlayer_Dark
      else -> R.style.Theme_BSPlayer
    }
  ...
  var materialPrimaryColor: Int
    get() = SPUtil.getValue(
      App.context, KEY_NAME, KEY_PRIMARY_COLOR, Color.TRANSPARENT
    )
    set(@ColorInt value) {
  ...
  val isLightTheme: Boolean
    get() = themeRes == R.style.Theme_BSPlayer

  private val isBlackTheme: Boolean
    get() = themeRes == R.style.Theme_BSPlayer_Black
}