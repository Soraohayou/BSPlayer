...
import com.blueshark.music.request.network.HttpClient
import com.blueshark.music.util.ImageUriUtil
import com.blueshark.music.util.MediaStoreUtil.getSongs
...
//  fun clearCache(model: BSPlayerModel) {
...
//      Timber.v("get from sp, uri: $uri")
  ...
  private fun downloadFromLastFM(): Boolean {
    return SPUtil.getValue(context, SPUtil.SETTING_KEY.NAME, SPUtil.SETTING_KEY.ALBUM_COVER_DOWNLOAD_SOURCE, DOWNLOAD_NETEASE) == DOWNLOAD_LASTFM
  }

  private fun canDownloadCover(): Boolean {
    val current = SPUtil.getValue(context, SPUtil.SETTING_KEY.NAME, SPUtil.SETTING_KEY.AUTO_DOWNLOAD_ALBUM_COVER, context.getString(R.string.never))
